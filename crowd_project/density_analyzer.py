...
mean ± σ thresholds (configurable in config.py).
"""
...
class DensityAnalyzer:
    ...
    def classify_batch(
        ...
        low_sigma: float = config.DENSITY_LOW_SIGMA,
        high_sigma: float = config.DENSITY_HIGH_SIGMA,
    ) -> None:
        """
        ...
        for m in metrics_list:
            if m.density_ratio < low_thresh:
                m.classification = "Low Crowd"
            elif m.density_ratio > high_thresh:
        ...
        logger.info(
            ...
            mu, sigma, low_thresh, high_thresh,
        )
